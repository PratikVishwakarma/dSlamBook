...
//                        imageView_otherUser_profile_pic.setImageBitmap(circularBitmap);
    ...
    public void sentMessage(final String message) {
        ...
        sentKey = chatRef.child(userIdByEmail).child(otherUserIdByEmail).push().getKey();
        recieveKey = chatRef.child(userIdByEmail).child(otherUserIdByEmail).push().getKey();
        sentChat = new Chat(userIdByEmail, otherUserIdByEmail, time, date, Constant.CHAT_MESSAGE_TYPE_SENT, message, "sent");
        recieveChat = new Chat(otherUserIdByEmail, userIdByEmail, time, date, Constant.CHAT_MESSAGE_TYPE_RECIEVE, message, "getMessage");
